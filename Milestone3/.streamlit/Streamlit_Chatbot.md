...
Description: Claude should use this skill as reference on how to create working chat user interfaces for Milestone 3. This is a chat interface that allows the user to query directly from Ollama. Implement this along with add from src.pipeline import run_query to call it when user make sa submission. Use it as reference to replace rule_based_framwork_row() swap wuth real LLM extraction from run_query()
use load_dotenv() to load Qdrant Cloud cREDENTIALS . !!!!IMPORTANT WARNING. This reference file uses langchain OpenAI framework to create the AI Agent use by the chatbot. Instead of OpenAI, use Langchain Ollama module and ChatOllama tool to define the Ollama agents of this project 
### IMPORT DEPENDENCIES ###
